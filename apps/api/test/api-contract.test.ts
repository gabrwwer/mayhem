...
function callHandler(handler: any, req: any) {
  return new Promise((resolve) => {
    const res = {
      ...
      json(payload: any) { resolve({ status: this._status ?? 200, body: payload }); },
...
describe('API contract (basic)', () => {
  ...
  it('GET /api/events returns array', async () => {
    const result = await callHandler(routes.getEvents, { query: {} });
  ...
  it('POST /api/positions/:id/close simulates close in dry run', async () => {
    ...
    const result = await callHandler(routes.closePosition, { params: { id: 'p1' } });
    expect(result.status).toBe(200);
  ...
  it('POST /api/positions/:id/partial-close validates input', async () => {
    ...
    const ok = await callHandler(routes.partialClose, { params: { id: 'p2' }, body: { quantity: 4 } });
  ...
  it('GET /api/portfolio returns DATA_INSUFFICIENT when valuation missing', async () => {
    ...
    const result = await callHandler(routes.getPortfolio, {});
    expect(result.status).toBe(200);
